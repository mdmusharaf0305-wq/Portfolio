    ...
    <Section title="Skills" id="skills" className="relative">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {SKILLS.map((skill) => (
          <div
            key={skill.key}
            className="perspective-1000 group cursor-pointer h-52 sm:h-48 md:h-44"
          >
            <div className="relative w-full h-full transform-style-3d duration-500 group-hover:rotate-y-180">
              ...
              {/* BACK FACE */}
              <div className="absolute inset-0 flex items-center justify-center p-3 sm:p-4 rounded-2xl border border-theme-border bg-theme-card backdrop-blur-md rotate-y-180 backface-hidden shadow-sm">
                <div className="flex flex-wrap gap-1 sm:gap-1.5 justify-center">
                  {skill.items.map((item, i) => (
                    <span
                      key={i}
                      className="text-[9px] sm:text-[10px] md:text-xs font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-brand-start/5 text-brand-start border border-brand-start/10 dark:bg-brand-end/5 dark:text-brand-end dark:border-brand-end/10 shadow-sm"
                    >
                      {item}
                    </span>
                  ))}
